...
def find(
    ...
    if refresh_database:
        ...
        for current_representation in representations:
            ...
            if alpha_hash != beta_hash:
                logger.debug(f"Even though {
                             identity} represented before, it's replaced later.")
    ...
    if must_save_pickle:
        ...
        if not silent:
            logger.info(f"There are now {
                        len(representations)} representations in {file_name}")
...
def __find_bulk_embeddings(
    ...
    for employee in tqdm(
        ...
        try:
            ...
            logger.error(f"Exception while extracting faces from {
                         employee}: {str(err)}")
            img_objs = []
